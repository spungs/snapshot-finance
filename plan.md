...
Phase 1 완료 후:
- [x] **구독 모델 설계 및 DB 스키마 변경** (User, Subscription Plan)
- [x] **스냅샷 제한 로직 구현** (Free: 30개, Pro: 125개, Max: 250개)
- [x] **자동 스냅샷 스케줄러** (유료 회원 전용 기능)
- [ ] 시뮬레이션 기능 ("팔지 않았다면?")
- [ ] Vercel 배포
- [ ] **한국투자증권(KIS) API 연동** (현재 진행 중)
  - [ ] 인증 토큰 발급 및 관리
  - [ ] 국내/해외 주식 시세 조회
  - [ ] (선택) 주식 검색 기능 대체 여부 검토
- [ ] **로그인/회원가입 구현** (Auth)
- [ ] **전역 상태 관리 도입** (Zustand)
