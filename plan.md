...
65: - [x] **예수금(Cash Balance) 관리**:
66:     - [x] Database Schema Update (User.cashBalance)
67:     - [x] UI: 대시보드에서 예수금 직접 수정 (천단위 포맷팅, 다국어/통화 자동 변환)
68:     - [x] UX: 수정 시 로딩 상태 표시 및 즉각적인 UI 반영
69:     - [x] 다국어 지원: 주식 평가액, 예수금, 평가손익(투자) 라벨 및 메시지 처리
