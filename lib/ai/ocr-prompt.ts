...
- ticker: 종목 코드. 영문 약자(미국: TSLA, AAPL, TSLL, USD, FBL) 또는 6자리 숫자(한국: 005930). 한국 종목은 6자리 숫자 형태가 명확히 보일 때만. 캡쳐에 종목명과 별개의 코드 표기가 있으면 반드시 추출 — KIS 매칭 정확도가 크게 올라간다.
...
- 종목명과 티커가 두 줄로 분리되어 있을 수 있음 (예: 위쪽 "디렉시온 테슬라 2배 ETF", 아래쪽 "TSLL"). 둘 다 추출하세요. 통화 표시("KRW", "USD")가 티커 자리에 있어도 그것이 실제 ETF 티커(예: USD = ProShares Ultra Semiconductors)일 수 있으니 그대로 추출하세요.
        ...
        holdings: {
            ...
            items: {
                ...
                properties: {
                    ...
                    ticker: { type: SchemaType.STRING, description: '종목 코드 (영문 약자 예: TSLA, AAPL / 한국 6자리 예: 005930). 캡쳐에 보이면 추출, 없으면 비워둠.' },
...
export type OcrHoldingItem = {
    stockName: string
    ticker?: string         // 종목 코드 (영문 약자 또는 6자리 한국 종목코드)
    quantity: number
    averagePrice?: number
    currency?: 'KRW' | 'USD'
    purchaseRate?: number
