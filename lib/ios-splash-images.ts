// PWA-Asset-Generator 가 생성한 iOS 디바이스별 splash 이미지 매니페스트.
// public/splash/ 의 PNG 파일을 device-width / device-height / pixel-ratio /
// orientation 조합으로 매핑한다. iOS 가 PWA 콜드 스타트 시 device 에 맞는
// splash 를 자동 선택해 검은 화면 대신 브랜드 splash 를 표시.
//
// 재생성: npx pwa-asset-generator public/logo.png public/splash \
//   --background '#ffffff' --padding '30%' --no-favicon --icon-only false
//   --splash-only true
